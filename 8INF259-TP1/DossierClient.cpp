...
///+ X A N : ajouter un client C habitant ‡ l'adresse A et au numÈro N ‡ la liste chaÓnÈe.
void DossierClient::AjouterClient(char * nom, char * rue, int numero)
{
	...
	//CrÈer le nouveau client ‡ la fin
...
/// - X : supprimer un client X de la liste chaÓnÈe.
void DossierClient::SupprimerClient(char * nom)
{
	...
	if (ptr_courant->nom == nom) {
		...
		// Boucler pour demander les prochaines clÈs
...
///= X Y M : ajouter un message M envoyÈ du client X au client Y.
...
///& X Y : afficher le nombre de messages Èchanger entre le client X et Y.
